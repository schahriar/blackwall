...
function RunRuleBloc(rules, context, callback) {
    ...
    */
    var ParallelExecutionArray = _.map(rules, function(rule) {
        return _.bind(rule.func, context, (rule.name)?context.information['_' + rule.name]:context.information);
    })
    ...
    RunRuleBloc(this.policy.rules, session, function(error) {
        if(!error) {
            ...
            if (!_this.members[session.id]) _this.members[session.id] = session;
                ...
                _this.members[session.id].information = _.assign(_this.members[session.id].information, session);
