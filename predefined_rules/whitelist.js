...
module.exports = {
    name: 'whitelist',
    description: 'Limits Sessions Based on IP Address of the Client. If a matching IP Address or Range is found the session will be allowed otherwise it will be terminated. Ranges are based on CIDR http://wikipedia.org/wiki/Classless_Inter-Domain_Routing',
    func: function(options, local, callback){
        if(!options.get('whitelist')) return callback(null, true);
        ...
        if(!IP_MATCH_ALL(options.get('whitelist.address'), options.get('whitelist.range'), this.information.ip)) {
            callback("IP is not allowed!");
        }else{
            callback(null, true);
        }
    }
